// ======================================================================
//  File   : components/mimo_massive/tb/mimo_massive_tb_top.sv
//  Author : AVIK MAJUMDAR
//  Project: AVIK VIP FACTORY - O-RAN VIP Suite
//  Date   : 2026-08-09
//  Desc   : mimo_massive self-checking TESTBENCH (Lane-1): driver applies Z3 stimulus; SCOREBOARD checks legality + pack-vs-golden + round-trip; ASSERTIONS as procedural invariant checks with exercise counters; COVERAGE via COVROW monitor log; UVM-format reporter.
// ======================================================================
module mimo_massive_tb_top import mimo_massive_pkg::*; ;
  mimo_massive_hdr_t m_in, m_out; logic [63:0] m_wire; string vio;
  ...
  int unsigned r_ant_cfg;
  int unsigned r_num_layers;
  int unsigned r_tdd_cfg;
  int unsigned r_rank;
  int unsigned r_precoder_idx;
  int unsigned r_seq;
  int unsigned r_rsvd;
  initial begin
    ...
    m_rc=$fscanf(m_fd,"%h %h %h %h %h %h %h %h %h",r_version, r_ant_cfg, r_num_layers, r_tdd_cfg, r_rank, r_precoder_idx, r_seq, r_rsvd, r_gold);
    while(m_rc==9) begin
      m_in.version = r_version[3:0];
      m_in.ant_cfg = r_ant_cfg[7:0];
      m_in.num_layers = r_num_layers[3:0];
      m_in.tdd_cfg = r_tdd_cfg[2:0];
      m_in.rank = r_rank[3:0];
      m_in.precoder_idx = r_precoder_idx[7:0];
      m_in.seq = r_seq[7:0];
      m_in.rsvd = r_rsvd[24:0];
      m_wire=mimo_massive_pack(m_in); m_out=mimo_massive_unpack(m_wire);
      ...
      m_ex_leg++; vio=mimo_massive_first_violation(m_in);
      if(m_neg) begin
        ...
        $display("COVROW,ant_cfg=%0d,tdd_cfg=%0d,seq=%0d", m_out.ant_cfg, m_out.tdd_cfg, m_out.seq);
      end
      m_txns++;
      m_rc=$fscanf(m_fd,"%h %h %h %h %h %h %h %h %h",r_version, r_ant_cfg, r_num_layers, r_tdd_cfg, r_rank, r_precoder_idx, r_seq, r_rsvd, r_gold);
    end
