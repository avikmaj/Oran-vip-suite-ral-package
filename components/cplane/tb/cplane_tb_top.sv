// ======================================================================
//  File   : components/cplane/tb/cplane_tb_top.sv
//  Author : AVIK MAJUMDAR
//  Project: AVIK VIP FACTORY - O-RAN VIP Suite
//  Date   : 2026-08-09
//  Desc   : cplane self-checking TESTBENCH (Lane-1): driver applies Z3 stimulus; SCOREBOARD checks legality + pack-vs-golden + round-trip; ASSERTIONS as procedural invariant checks with exercise counters; COVERAGE via COVROW monitor log; UVM-format reporter.
// ======================================================================
module cplane_tb_top import cplane_pkg::*; ;
  cplane_hdr_t m_in, m_out; logic [63:0] m_wire; string vio;
  ...
  int unsigned r_section_ext;
  int unsigned r_start_symbol;
  int unsigned r_num_symbol;
  int unsigned r_num_sections;
  ...
  initial begin
    ...
    m_rc=$fscanf(m_fd,"%h %h %h %h %h %h %h %h %h %h %h",r_version, r_section_type, r_section_ext, r_start_symbol, r_num_symbol, r_num_sections, r_beam_id, r_numerology, r_seq, r_rsvd, r_gold);
    while(m_rc==11) begin
      m_in.version = r_version[3:0];
      m_in.section_type = r_section_type[3:0];
      m_in.section_ext = r_section_ext[3:0];
      m_in.start_symbol = r_start_symbol[3:0];
      m_in.num_symbol = r_num_symbol[3:0];
      m_in.num_sections = r_num_sections[3:0];
      m_in.beam_id = r_beam_id[15:0];
      m_in.numerology = r_numerology[3:0];
      m_in.seq = r_seq[7:0];
      m_in.rsvd = r_rsvd[11:0];
      m_wire=cplane_pack(m_in); m_out=cplane_unpack(m_wire);
      ...
      m_ex_leg++; vio=cplane_first_violation(m_in);
      if(m_neg) begin
        ...
        $display("COVROW,section_type=%0d,section_ext=%0d,num_symbol=%0d,num_sections=%0d,numerology=%0d,seq=%0d", m_out.section_type, m_out.section_ext, m_out.num_symbol, m_out.num_sections, m_out.numerology, m_out.seq);
      end
      m_txns++;
      m_rc=$fscanf(m_fd,"%h %h %h %h %h %h %h %h %h %h %h",r_version, r_section_type, r_section_ext, r_start_symbol, r_num_symbol, r_num_sections, r_beam_id, r_numerology, r_seq, r_rsvd, r_gold);
    end
