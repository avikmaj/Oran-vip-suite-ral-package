// ======================================================================
//  File   : components/prach/tb/prach_tb_top.sv
//  Author : AVIK MAJUMDAR
//  Project: AVIK VIP FACTORY - O-RAN VIP Suite
//  Date   : 2026-08-09
//  Desc   : prach self-checking TESTBENCH (Lane-1): driver applies Z3 stimulus; SCOREBOARD checks legality + pack-vs-golden + round-trip; ASSERTIONS as procedural invariant checks with exercise counters; COVERAGE via COVROW monitor log; UVM-format reporter.
// ======================================================================
module prach_tb_top import prach_pkg::*; ;
  prach_hdr_t m_in, m_out; logic [63:0] m_wire; string vio;
  ...
  int unsigned r_format;
  int unsigned r_numerology;
  int unsigned r_fr;
  int unsigned r_zc_root;
  int unsigned r_occasion;
  int unsigned r_seq;
  int unsigned r_rsvd;
  initial begin
    ...
    m_rc=$fscanf(m_fd,"%h %h %h %h %h %h %h %h %h %h",r_version, r_section_type, r_format, r_numerology, r_fr, r_zc_root, r_occasion, r_seq, r_rsvd, r_gold);
    while(m_rc==10) begin
      m_in.version = r_version[3:0];
      m_in.section_type = r_section_type[3:0];
      m_in.format = r_format[3:0];
      m_in.numerology = r_numerology[3:0];
      m_in.fr = r_fr[0:0];
      m_in.zc_root = r_zc_root[7:0];
      m_in.occasion = r_occasion[5:0];
      m_in.seq = r_seq[7:0];
      m_in.rsvd = r_rsvd[24:0];
      m_wire=prach_pack(m_in); m_out=prach_unpack(m_wire);
      ...
      m_ex_leg++; vio=prach_first_violation(m_in);
      if(m_neg) begin
        ...
        $display("COVROW,format=%0d,numerology=%0d,fr=%0d,seq=%0d", m_out.format, m_out.numerology, m_out.fr, m_out.seq);
      end
      m_txns++;
      m_rc=$fscanf(m_fd,"%h %h %h %h %h %h %h %h %h %h",r_version, r_section_type, r_format, r_numerology, r_fr, r_zc_root, r_occasion, r_seq, r_rsvd, r_gold);
    end
