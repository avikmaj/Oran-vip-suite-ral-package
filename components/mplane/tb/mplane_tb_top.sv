// ======================================================================
//  File   : components/mplane/tb/mplane_tb_top.sv
//  Author : AVIK MAJUMDAR
//  Project: AVIK VIP FACTORY - O-RAN VIP Suite
//  Date   : 2026-08-09
//  Desc   : mplane self-checking TESTBENCH (Lane-1): driver applies Z3 stimulus; SCOREBOARD checks legality + pack-vs-golden + round-trip; ASSERTIONS as procedural invariant checks with exercise counters; COVERAGE via COVROW monitor log; UVM-format reporter.
// ======================================================================
module mplane_tb_top import mplane_pkg::*; ;
  mplane_hdr_t m_in, m_out; logic [63:0] m_wire; string vio;
  ...
  int unsigned r_netconf_op;
  int unsigned r_datastore;
  int unsigned r_item_id;
  int unsigned r_ant_cal;
  int unsigned r_sw_slot;
  int unsigned r_seq;
  int unsigned r_rsvd;
  initial begin
    ...
    m_rc=$fscanf(m_fd,"%h %h %h %h %h %h %h %h %h",r_version, r_netconf_op, r_datastore, r_item_id, r_ant_cal, r_sw_slot, r_seq, r_rsvd, r_gold);
    while(m_rc==9) begin
      m_in.version = r_version[3:0];
      m_in.netconf_op = r_netconf_op[2:0];
      m_in.datastore = r_datastore[1:0];
      m_in.item_id = r_item_id[15:0];
      m_in.ant_cal = r_ant_cal[0:0];
      m_in.sw_slot = r_sw_slot[2:0];
      m_in.seq = r_seq[7:0];
      m_in.rsvd = r_rsvd[26:0];
      m_wire=mplane_pack(m_in); m_out=mplane_unpack(m_wire);
      ...
      m_ex_leg++; vio=mplane_first_violation(m_in);
      if(m_neg) begin
        ...
        $display("COVROW,netconf_op=%0d,datastore=%0d,ant_cal=%0d,seq=%0d", m_out.netconf_op, m_out.datastore, m_out.ant_cal, m_out.seq);
      end
      m_txns++;
      m_rc=$fscanf(m_fd,"%h %h %h %h %h %h %h %h %h",r_version, r_netconf_op, r_datastore, r_item_id, r_ant_cal, r_sw_slot, r_seq, r_rsvd, r_gold);
    end
