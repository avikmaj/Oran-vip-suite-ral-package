// ======================================================================
//  File   : components/compression/tb/compression_tb_top.sv
//  Author : AVIK MAJUMDAR
//  Project: AVIK VIP FACTORY - O-RAN VIP Suite
//  Date   : 2026-08-09
//  Desc   : compression self-checking TESTBENCH (Lane-1): driver applies Z3 stimulus; SCOREBOARD checks legality + pack-vs-golden + round-trip; ASSERTIONS as procedural invariant checks with exercise counters; COVERAGE via COVROW monitor log; UVM-format reporter.
// ======================================================================
module compression_tb_top import compression_pkg::*; ;
  compression_hdr_t m_in, m_out; logic [63:0] m_wire; string vio;
  ...
  int unsigned r_method;
  int unsigned r_iq_width;
  int unsigned r_comp_width;
  int unsigned r_exponent;
  int unsigned r_block_size;
  int unsigned r_seq;
  int unsigned r_rsvd;
  initial begin
    ...
    m_rc=$fscanf(m_fd,"%h %h %h %h %h %h %h %h %h",r_version, r_method, r_iq_width, r_comp_width, r_exponent, r_block_size, r_seq, r_rsvd, r_gold);
    while(m_rc==9) begin
      m_in.version = r_version[3:0];
      m_in.method = r_method[1:0];
      m_in.iq_width = r_iq_width[4:0];
      m_in.comp_width = r_comp_width[4:0];
      m_in.exponent = r_exponent[3:0];
      m_in.block_size = r_block_size[7:0];
      m_in.seq = r_seq[7:0];
      m_in.rsvd = r_rsvd[27:0];
      m_wire=compression_pack(m_in); m_out=compression_unpack(m_wire);
      ...
      m_ex_leg++; vio=compression_first_violation(m_in);
      if(m_neg) begin
        ...
        $display("COVROW,method=%0d,comp_width=%0d,seq=%0d", m_out.method, m_out.comp_width, m_out.seq);
      end
      m_txns++;
      m_rc=$fscanf(m_fd,"%h %h %h %h %h %h %h %h %h",r_version, r_method, r_iq_width, r_comp_width, r_exponent, r_block_size, r_seq, r_rsvd, r_gold);
    end
