// ======================================================================
//  File   : components/bwp/tb/bwp_tb_top.sv
//  Author : AVIK MAJUMDAR
//  Project: AVIK VIP FACTORY - O-RAN VIP Suite
//  Date   : 2026-08-09
//  Desc   : bwp self-checking TESTBENCH (Lane-1): driver applies Z3 stimulus; SCOREBOARD checks legality + pack-vs-golden + round-trip; ASSERTIONS as procedural invariant checks with exercise counters; COVERAGE via COVROW monitor log; UVM-format reporter.
// ======================================================================
module bwp_tb_top import bwp_pkg::*; ;
  bwp_hdr_t m_in, m_out; logic [63:0] m_wire; string vio;
  ...
  int unsigned r_bwp_id;
  int unsigned r_numerology;
  int unsigned r_bw_mhz;
  int unsigned r_fr;
  int unsigned r_active;
  int unsigned r_seq;
  int unsigned r_rsvd;
  initial begin
    ...
    m_rc=$fscanf(m_fd,"%h %h %h %h %h %h %h %h %h",r_version, r_bwp_id, r_numerology, r_bw_mhz, r_fr, r_active, r_seq, r_rsvd, r_gold);
    while(m_rc==9) begin
      m_in.version = r_version[3:0];
      m_in.bwp_id = r_bwp_id[1:0];
      m_in.numerology = r_numerology[3:0];
      m_in.bw_mhz = r_bw_mhz[8:0];
      m_in.fr = r_fr[0:0];
      m_in.active = r_active[0:0];
      m_in.seq = r_seq[7:0];
      m_in.rsvd = r_rsvd[34:0];
      m_wire=bwp_pack(m_in); m_out=bwp_unpack(m_wire);
      ...
      m_ex_leg++; vio=bwp_first_violation(m_in);
      if(m_neg) begin
        ...
        $display("COVROW,bwp_id=%0d,numerology=%0d,bw_mhz=%0d,fr=%0d,active=%0d,seq=%0d", m_out.bwp_id, m_out.numerology, m_out.bw_mhz, m_out.fr, m_out.active, m_out.seq);
      end
      m_txns++;
      m_rc=$fscanf(m_fd,"%h %h %h %h %h %h %h %h %h",r_version, r_bwp_id, r_numerology, r_bw_mhz, r_fr, r_active, r_seq, r_rsvd, r_gold);
    end
