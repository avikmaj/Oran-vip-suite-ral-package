// ======================================================================
//  File   : components/beamforming/tb/beamforming_tb_top.sv
//  Author : AVIK MAJUMDAR
//  Project: AVIK VIP FACTORY - O-RAN VIP Suite
//  Date   : 2026-08-09
//  Desc   : beamforming self-checking TESTBENCH (Lane-1): driver applies Z3 stimulus; SCOREBOARD checks legality + pack-vs-golden + round-trip; ASSERTIONS as procedural invariant checks with exercise counters; COVERAGE via COVROW monitor log; UVM-format reporter.
// ======================================================================
module beamforming_tb_top import beamforming_pkg::*; ;
  beamforming_hdr_t m_in, m_out; logic [63:0] m_wire; string vio;
  ...
  int unsigned r_num_ports;
  int unsigned r_num_layers;
  int unsigned r_section_ext;
  int unsigned r_codebook_idx;
  int unsigned r_seq;
  int unsigned r_rsvd;
  initial begin
    ...
    m_rc=$fscanf(m_fd,"%h %h %h %h %h %h %h %h %h",r_version, r_beam_id, r_num_ports, r_num_layers, r_section_ext, r_codebook_idx, r_seq, r_rsvd, r_gold);
    while(m_rc==9) begin
      m_in.version = r_version[3:0];
      m_in.beam_id = r_beam_id[15:0];
      m_in.num_ports = r_num_ports[7:0];
      m_in.num_layers = r_num_layers[3:0];
      m_in.section_ext = r_section_ext[3:0];
      m_in.codebook_idx = r_codebook_idx[15:0];
      m_in.seq = r_seq[7:0];
      m_in.rsvd = r_rsvd[3:0];
      m_wire=beamforming_pack(m_in); m_out=beamforming_unpack(m_wire);
      ...
      m_ex_leg++; vio=beamforming_first_violation(m_in);
      if(m_neg) begin
        ...
        $display("COVROW,num_ports=%0d,section_ext=%0d,seq=%0d", m_out.num_ports, m_out.section_ext, m_out.seq);
      end
      m_txns++;
      m_rc=$fscanf(m_fd,"%h %h %h %h %h %h %h %h %h",r_version, r_beam_id, r_num_ports, r_num_layers, r_section_ext, r_codebook_idx, r_seq, r_rsvd, r_gold);
    end
