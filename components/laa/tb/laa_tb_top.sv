// ======================================================================
//  File   : components/laa/tb/laa_tb_top.sv
//  Author : AVIK MAJUMDAR
//  Project: AVIK VIP FACTORY - O-RAN VIP Suite
//  Date   : 2026-08-09
//  Desc   : laa self-checking TESTBENCH (Lane-1): driver applies Z3 stimulus; SCOREBOARD checks legality + pack-vs-golden + round-trip; ASSERTIONS as procedural invariant checks with exercise counters; COVERAGE via COVROW monitor log; UVM-format reporter.
// ======================================================================
module laa_tb_top import laa_pkg::*; ;
  laa_hdr_t m_in, m_out; logic [63:0] m_wire; string vio;
  ...
  int unsigned r_lbt_cat;
  int unsigned r_lbt_result;
  int unsigned r_burst_type;
  int unsigned r_cap;
  ...
  initial begin
    ...
    m_rc=$fscanf(m_fd,"%h %h %h %h %h %h %h %h %h %h",r_version, r_section_type, r_lbt_cat, r_lbt_result, r_burst_type, r_cap, r_section_ext, r_seq, r_rsvd, r_gold);
    while(m_rc==10) begin
      m_in.version = r_version[3:0];
      m_in.section_type = r_section_type[3:0];
      m_in.lbt_cat = r_lbt_cat[2:0];
      m_in.lbt_result = r_lbt_result[0:0];
      m_in.burst_type = r_burst_type[0:0];
      m_in.cap = r_cap[1:0];
      m_in.section_ext = r_section_ext[3:0];
      m_in.seq = r_seq[7:0];
      m_in.rsvd = r_rsvd[36:0];
      m_wire=laa_pack(m_in); m_out=laa_unpack(m_wire);
      ...
      m_ex_leg++; vio=laa_first_violation(m_in);
      if(m_neg) begin
        ...
        $display("COVROW,lbt_cat=%0d,lbt_result=%0d,burst_type=%0d,cap=%0d,seq=%0d", m_out.lbt_cat, m_out.lbt_result, m_out.burst_type, m_out.cap, m_out.seq);
      end
      m_txns++;
      m_rc=$fscanf(m_fd,"%h %h %h %h %h %h %h %h %h %h",r_version, r_section_type, r_lbt_cat, r_lbt_result, r_burst_type, r_cap, r_section_ext, r_seq, r_rsvd, r_gold);
    end
